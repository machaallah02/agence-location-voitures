                    ...
                    <div class="row">
                        <div class="col-md-5 text-center">
                            @if($reservation->vehicule->image)
                                <img src="{{ asset('storage/'.$reservation->vehicule->image) }}" class="img-fluid rounded" alt="Image du véhicule">
                            @else
                                <img src="https://via.placeholder.com/300" class="img-fluid rounded" alt="Image par défaut du véhicule">
                            @endif
                        </div>
                        <div class="col-md-7">
                            ...
                            <p class="card-text"><strong>Numéro d'immatriculation:</strong> {{ $reservation->vehicule->numéro_immatriculation }}</p>

                            <h5 class="card-title mt-4">Réservation</h5>
                            <p class="card-text"><strong>Date de début:</strong> {{ $reservation->date_debut->format('d/m/Y') }}</p>
                            <p class="card-text"><strong>Date de fin:</strong> {{ $reservation->date_fin->format('d/m/Y H:i') }}</p>
                            <p class="card-text"><strong>Coût Total:</strong> {{ number_format($reservation->coût_total, 2, ',', ' ') }} franc CFA</p>
                            <p class="card-text"><strong>Date de réservation:</strong> {{ $reservation->created_at->format('d/m/Y H:i') }}</p>
